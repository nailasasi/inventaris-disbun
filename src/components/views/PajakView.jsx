    ...
    <div className={`rounded-[2rem] md:rounded-[3rem] shadow-xl border overflow-hidden ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white'}`}>
      <div className={`p-6 md:p-8 border-b flex flex-col md:flex-row justify-between items-start md:items-center gap-4 ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50'}`}>
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          ...
          <select value={pajakSelectedYear} onChange={e => setPajakSelectedYear(e.target.value)} className={`p-2 font-bold text-sm outline-none border rounded-xl ${isDarkMode ? 'bg-slate-700 border-slate-600' : 'bg-white border-slate-200'}`}>
            {pajakYearsList.map(y => <option key={y} value={y}>{y}</option>)}
          </select>
          <button
            onClick={() => {
                          ...
                          <button
                           onClick={async () => {
                              ...
                              try {
                                ...
                                const ext = cleanUrl.substring(cleanUrl.lastIndexOf('.') + 1) || 'file';
