...
const App = () => {
  ...
  const [linkAsetModal, setLinkAsetModal] = useState({
    show: false,
    item: null,
  });
  const [editNilaiModal, setEditNilaiModal] = useState({
    show: false,
    item: null,
  });
  const [editPajakModal, setEditPajakModal] = useState({
    show: false,
    item: null,
  });
  ...
  const [pajakSelectedYear, setPajakSelectedYear] = useState(
    new Date().getFullYear().toString(),
  );
  const [pbbSelectedYear, setPbbSelectedYear] = useState(
    new Date().getFullYear().toString(),
  );
  const [pajakYearsList, setPajakYearsList] = useState([
    "2025",
    "2026",
    "2027",
    "2028",
  ...
  const [pbbYearsList, setPbbYearsList] = useState([
    "2025",
    "2026",
    "2027",
    "2028",
  ...
  const [editTanahModal, setEditTanahModal] = useState({
    show: false,
    item: null,
  });
  const [mediaTanahModal, setMediaTanahModal] = useState({
    show: false,
    item: null,
    file: null,
    preview: null,
  ...
  const filteredRuanganData = useMemo(
    () =>
      ruanganData.filter(
        (r) => (r.unitId || UNIT_CODES.PUSAT) === activeUnitView,
      ),
    [ruanganData, activeUnitView],
  );
  ...
  const filteredPegawaiData = useMemo(() => {
    return (data || []).filter((p) => {
      ...
      const matchP2BTP =
        str.includes(
          "UPT PENGEMBANGAN DAN PRODUKSI BENIH TANAMAN PERKEBUNAN",
        ) || str.includes("P2BTP");
      const matchPSBP =
        str.includes("UPT PENGAWASAN DAN SERTIFIKASI BENIH PERKEBUNAN") ||
        str.includes("PSBP");
  ...
  const filteredHistoryData = useMemo(
    () =>
      historyData.filter(
        (h) => (h.unitId || UNIT_CODES.PUSAT) === activeUnitView,
      ),
    [historyData, activeUnitView],
  );
  const baseRooms = activeUnitView === UNIT_CODES.PUSAT ? ROOM_LIST : [];
  const dynamicRooms = Array.from(
    new Set([
      ...baseRooms,
      ...(filteredRuanganData || []).map((r) => r.nama_ruangan),
    ]),
  ).filter(Boolean);
  ...
  const allowedBidangList =
    userRole === "admin"
      ? ALL_BIDANG_RKBMD
      : userRole === "admin_upt" && userUnit === UNIT_CODES.P2BTP
        ? [UNIT_LABELS[UNIT_CODES.P2BTP]]
        : userRole === "admin_upt" && userUnit === UNIT_CODES.PSBP
          ? [UNIT_LABELS[UNIT_CODES.PSBP]]
          : userRole === "pph"
  ...
  const visibleRkbmdData = (filteredRkbmdData || []).filter((r) => {
    ...
    if (userRole === "pph")
      return r.bidang === "Bidang Produksi dan Pemasaran Hasil (PPH)";
    if (userRole === "perlinbun") return r.bidang === "Bidang Perlindungan";
    if (userRole === "semusim") return r.bidang === "Bidang Tanaman Semusim";
    if (userRole === "tahunan")
      return r.bidang === "Bidang Proteksi Tanaman Tahunan";
    return false;
  });
  const displayedRkbmdData = visibleRkbmdData.filter((r) => {
    const matchBidang =
      rkbmdFilters.length === 0 || rkbmdFilters.includes(r.bidang);
    const matchJenis =
      rkbmdSelectedJenis === "Semua" || r.jenis_usulan === rkbmdSelectedJenis;
    const matchTahun =
      rkbmdSelectedYear === "Semua" ||
      safeString(r.tahun_usulan) === rkbmdSelectedYear;
  ...
  const customConfirm = (title, message, type, onConfirm) =>
    setConfirmModal({
      show: true,
      title: safeString(title),
      message: safeString(message),
  ...
  const generateBAST = async (item, sender, receiver) => {
    try {
      ...
      const barang = buildBastBarangData([item], priceData, sender, safeString);
  ...
  const generateBASTAntarUnit = (
    ...
    if (window.saveAs)
      window.saveAs(
        blob,
        `BAST_AntarUnit_${safeString(item.no_kartu).replace(/[^a-zA-Z0-9.-]/g, "_")}.doc`,
      );
    else {
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      a.download = `BAST_AntarUnit_${safeString(item.no_kartu).replace(/[^a-zA-Z0-9.-]/g, "_")}.doc`;
      a.click();
    }
  };
  ...
  const generateBASTKendaraan = async (item, sender, receiver) => {
    ...
    try {
      ...
      const kendaraan = buildBastKendaraanData([item], safeString);
      ...
      customAlert("Export gagal", err.message);
    }
  };

  const generateSPPKD = async (kendaraan, pengemudi, pengurus, tujuan) => {
    try {
      if (!templates.sppkd?.url) {
        customAlert("Template belum ada", "Silakan upload template SPPKD.");
        return;
      }
      ...
      const dataTemplate = {
        ...
        no_polisi: safeString(kendaraan.no_polisi || kendaraan.no_kartu || "-"),
  ...
  const monitoringDataFiltered = useMemo(() => {
    ...
    Object.values(filteredPriceData || {}).forEach((item) => {
      ...
      if (expDate && !isNaN(expDate)) {
        ...
        diff = Math.ceil((expDate - today) / 86400000);
        expired = diff <= 0;
      }
      ...
      if (loc && loc !== "-" && loc !== "N/A" && loc !== "Manual") {
        if (dynamicRooms.includes(loc)) ruangan = loc;
        else pemegang = loc;
      }
      ...
      const liveNilaiBuku = item.manual_active
        ? item.nilai_buku
        : calculateNilaiBuku(
    ...
    console.log("ITEM PEGAWAI PERTAMA", filteredPegawaiData[0]?.items);
    ...
    (filteredPegawaiData || []).forEach((p) => {
      (p.items || []).forEach((it, idx) => {
        ...
        const existsInAny = Object.values(priceData).find(
          (x) => x.no_kartu?.toLowerCase() === cleanNo,
        );
        if (existsInAny) {
          if ((existsInAny.unitId || UNIT_CODES.PUSAT) !== activeUnitView)
            return;
        } else {
          if (activeUnitView !== UNIT_CODES.PUSAT) return;
        }

        if (!cleanNo) {
          const matchedKey = Object.keys(assetMap).find(
            (k) =>
              safeString(assetMap[k].nama).toLowerCase() ===
                safeString(it.nama).trim().toLowerCase() &&
              (safeString(assetMap[k].pemegang).toLowerCase() ===
                safeString(p.nama).toLowerCase() ||
                safeString(assetMap[k].lokasi).toLowerCase() ===
                  safeString(p.nama).toLowerCase()),
          );
          if (matchedKey) cleanNo = matchedKey;
        }
        if (cleanNo && assetMap[cleanNo]) {
          if (assetMap[cleanNo].nama === "TOYOTA KIJANG INNOVA GXW42") {
          ...
          if (dPengadaan) {
            ...
            dynTglHabis = dHabis.toISOString();
          }
        ...
        if (!cleanNo) {
          const matchedKey = Object.keys(assetMap).find(
            (k) =>
              safeString(assetMap[k].nama).toLowerCase() ===
                safeString(it.nama).trim().toLowerCase() &&
              (safeString(assetMap[k].ruangan).toLowerCase() ===
                safeString(r.nama_ruangan).toLowerCase() ||
                safeString(assetMap[k].lokasi).toLowerCase() ===
                  safeString(r.nama_ruangan).toLowerCase()),
          );
          if (matchedKey) cleanNo = matchedKey;
...
const disposalData = monitoringDataFiltered
  ...
  const totalInventaris =
    (filteredPegawaiData || []).reduce(
      (acc, p) => acc + (p.items?.length || 0),
      0,
    ) +
    (filteredRuanganData || []).reduce(
  ...
  const filteredTanahDataFinal = filteredTanahDataForView.filter(
    (t) =>
      ...
      safeString(t.deskripsi)
        .toLowerCase()
        .includes(tanahSearch.toLowerCase()) ||
      safeString(t.alamat).toLowerCase().includes(tanahSearch.toLowerCase()),
  );
  const statsAsetAktif = activeMonitoringData.filter(
    (x) => !x.manual_active,
  ).length;
  const statsReAktif = activeMonitoringData.filter(
    (x) => x.manual_active,
  ).length;
  ...
  const pengurusOptions = useMemo(() => {
    ...
    if (!opts.some((o) => safeString(o.nama).toLowerCase().includes("sugeng")))
      opts.push({ nama: "SUGENG RIYANTO, S.E.", nip: "19740204 200701 1 008" });
    if (!opts.some((o) => safeString(o.nama).toLowerCase().includes("achmar")))
      opts.push({
        nama: "ACHMAR ADRIAN RAMADHAN, A.Md.M.",
        nip: "19980126 202012 1 003",
      });
    return opts;
  }, [filteredPegawaiData]);
  const suratJalanHistory = filteredHistoryData.filter(
    (h) =>
      h.type === "SURAT JALAN" &&
      (!suratJalanForm.kendaraanId ||
        h.no_kartu === suratJalanForm.kendaraanId),
  );
  const availableVehicles = vehicleItems.filter((v) => {
    const isBooked = filteredHistoryData.some(
      (h) =>
        h.type === "SURAT JALAN" &&
        h.no_kartu === v.no_kartu &&
        (h.tglBerangkatRaw === suratJalanForm.tglBerangkat ||
          h.tglBerangkat ===
            new Date(suratJalanForm.tglBerangkat).toLocaleDateString("id-ID", {
              day: "numeric",
              month: "long",
              year: "numeric",
            })),
  ...
  const todayBookings = filteredHistoryData.filter((h) => {
    ...
    const diffHours =
      (new Date() - new Date(h.timestamp.seconds * 1000)) / (1000 * 60 * 60);
    return diffHours <= 24;
  });
  useEffect(() => {
    if (
      isLoggedIn &&
      (userRole === "admin" || userRole === "admin_upt") &&
      !taxAlertShown &&
      ...
      const expiringTaxes = Object.values(filteredPriceData).filter((item) => {
        ...
        const d =
          getSafeDateString(item.tgl_pajak) ||
          getSafeDateString(item.tgl_pengadaan);
        if (!d) return false;
      ...
      if (expiringTaxes.length > 0)
        customAlert(
          ...
          `Terdapat ${expiringTaxes.length} kendaraan dinas yang masa pajaknya akan habis di unit ini.`,
        );
  ...
  useEffect(() => {
    ...
    const unsubP = onSnapshot(
      collection(db, "artifacts", appId, "public", "data", "pegawai"),
      (s) => {
        const t = [];
        s.forEach((d) => t.push(d.data()));
        setData(t);
        setStatus((x) => ({ ...x, synced: true }));
        if (selectedUser)
          setSelectedUser(t.find((p) => p.nip === selectedUser.nip));
      },
      (err) => console.error("Error fetching pegawai:", err),
    );
    const unsubR = onSnapshot(
      collection(db, "artifacts", appId, "public", "data", "ruangan"),
      (s) => {
        const t = [];
  ...
  const unsubKB = onSnapshot(
    ...
    const unsubH = onSnapshot(
      collection(db, "artifacts", appId, "public", "data", "histori"),
      (s) => {
        const t = [];
        s.forEach((d) => t.push({ id: d.id, ...d.data() }));
        setHistoryData(
    ...
    const unsubMutasi = onSnapshot(
      collection(db, "artifacts", appId, "public", "data", "mutasi_pending"),
      (s) => {
        const t = [];
        s.forEach((d) => t.push({ id: d.id, ...d.data() }));
        setMutasiPendingData(t);
      },
      (err) => console.error("Error fetching mutasi:", err),
    );
    const unsubK = onSnapshot(
      collection(db, "artifacts", appId, "public", "data", "rkbmd"),
      (s) => {
        const t = [];
        s.forEach((d) => {
          const x = d.data();
          if (x.status !== "deleted") t.push({ id: d.id, ...x });
        });
        setRkbmdData(
          t.sort(
    ...
    const unsubT = onSnapshot(
      collection(db, "artifacts", appId, "public", "data", "tanah"),
      (s) => {
        const t = [];
        s.forEach((d) => t.push({ id: d.id, ...d.data() }));
        setTanahData(t);
    ...
    const unsubTemplates = onSnapshot(
      ...
      (s) => {
        if (s.exists()) setTemplates(s.data());
        else setTemplates({ sppbi: "", bast_barang: "", kir: "", label: "" });
      },
      (err) => console.error("Error fetching templates:", err),
    );
    const unsubRkbmdArsip = onSnapshot(
      collection(db, "artifacts", appId, "public", "data", "rkbmd_arsip"),
      (s) => {
        const t = {};
        s.forEach((d) => (t[d.id] = d.data().fileUrl));
        setRkbmdArsip(t);
      },
  ...
  const handleLogin = (e) => {
    ...
    if (
      ...
      loginCreds.password === "Asetdisbun123"
    ) {
      setUserRole("admin");
      setUserUnit(UNIT_CODES.PUSAT);
      setActiveUnitView(UNIT_CODES.PUSAT);
      setIsLoggedIn(true);
      setLoginError("");
      setShowDashboardAlert(true);
    } else if (
      loginCreds.username === "admin" &&
      loginCreds.password === "Asetp2btp"
    ) {
      setUserRole("admin_upt");
      setUserUnit(UNIT_CODES.P2BTP);
      setActiveUnitView(UNIT_CODES.P2BTP);
      setIsLoggedIn(true);
      setLoginError("");
      setShowDashboardAlert(true);
    } else if (
      loginCreds.username === "admin" &&
      loginCreds.password === "Asetpsbp"
    ) {
      setUserRole("admin_upt");
      setUserUnit(UNIT_CODES.PSBP);
      setActiveUnitView(UNIT_CODES.PSBP);
      setIsLoggedIn(true);
      setLoginError("");
      setShowDashboardAlert(true);
      ...
      setUserUnit(UNIT_CODES.PUSAT);
      setActiveUnitView(UNIT_CODES.PUSAT);
      if (loginCreds.password === "adminpph") {
        setUserRole("pph");
        setIsLoggedIn(true);
        setLoginError("");
        setShowDashboardAlert(true);
      } else if (loginCreds.password === "adminperlinbun") {
        setUserRole("perlinbun");
        setIsLoggedIn(true);
        setLoginError("");
        setShowDashboardAlert(true);
      } else if (loginCreds.password === "adminsemusim") {
        setUserRole("semusim");
        setIsLoggedIn(true);
        setLoginError("");
        setShowDashboardAlert(true);
      } else if (loginCreds.password === "admintahunan") {
        setUserRole("tahunan");
        setIsLoggedIn(true);
        setLoginError("");
        setShowDashboardAlert(true);
      ...
      const staffUser = data.find(
        (p) =>
          safeString(p.nip).replace(/\s/g, "") ===
          loginCreds.username.replace(/\s/g, ""),
      );
      if (staffUser && loginCreds.password === "disbun") {
        setUserRole("staff");
        const str = JSON.stringify(staffUser).toUpperCase();
        if (
          str.includes("PENGAWASAN DAN SERTIFIKASI") ||
          str.includes("PSBP")
        ) {
          setUserUnit(UNIT_CODES.PSBP);
          setActiveUnitView(UNIT_CODES.PSBP);
        } else if (
          str.includes("PENGEMBANGAN DAN PRODUKSI") ||
          str.includes("P2BTP")
        ) {
          setUserUnit(UNIT_CODES.P2BTP);
          setActiveUnitView(UNIT_CODES.P2BTP);
  ...
  const handleSearchChange = (e) => {
    ...
    if (val.length > 2) {
      ...
      const found = filteredPegawaiData.find(
        (p) =>
          safeString(p.nip).replace(/\s/g, "").includes(clean) ||
          safeString(p.nama).toLowerCase().includes(val.toLowerCase()),
      );
      ...
      try {
        ...
        if (status.uploadType === "prices") {
          for (let i = 1; i < jsonData.length; i++) {
            ...
            const noKartu = safeString(row[0]).trim();
            if (!noKartu) continue;

            const tglPengadaanVal =
            ...
            const perolehanVal =
              typeof row[3] === "number"
                ? row[3]
                : parseFloat(safeString(row[3]).replace(/,/g, "")) || 0;

            const { masa_manfaat } = parseKodeBarangData(
              noKartu,
              kodeBarangData,
            );
            const calculatedNilaiBuku = calculateNilaiBuku(
              perolehanVal,
              tglPengadaanVal,
              masa_manfaat,
            );
            let calculatedTglHabis =
            ...
            if (
              dPengadaan &&
              (!calculatedTglHabis ||
                calculatedTglHabis.trim() === "" ||
                calculatedTglHabis === "undefined")
            ) {
              const dHabis = new Date(dPengadaan);
              dHabis.setFullYear(dHabis.getFullYear() + masa_manfaat);
            ...
            const payload = {
              ...injectUnit,
              nama: safeString(row[8]).trim().toUpperCase(),
              no_kartu: noKartu,
              tgl_pengadaan: tglPengadaanVal,
              tgl_habis: calculatedTglHabis,
              nilai_perolehan: perolehanVal,
              nilai_buku: calculatedNilaiBuku,
        ...
        for (let i = startRow; i < jsonData.length; i++) {
        ...
        } if (status.uploadType === "tanah") {
          let headers = [],
            startRow = 0;
          for (let r = 0; r < Math.min(5, jsonData.length); r++) {
            const rowStr = (jsonData[r] || [])
              .map((c) => safeString(c).toLowerCase())
              .join(" ");
            if (
              rowStr.includes("kib") ||
              rowStr.includes("kode") ||
              rowStr.includes("luas") ||
              rowStr.includes("alamat")
            ) {
              headers = (jsonData[r] || []).map((c) =>
          ...
          for (let i = startRow; i < jsonData.length; i++) {
            ...
            if (
              !kib ||
              kib.toLowerCase().includes("kib") ||
              kib.toLowerCase().includes("nomor")
            )
              continue;
            const safeId = kib.replace(/[^a-zA-Z0-9.-]/g, "_");
            batch.set(
              doc(db, "artifacts", appId, "public", "data", "tanah", safeId),
              {
                ...injectUnit,
                kib,
                deskripsi: safeString(row[colNama]),
                tgl_peroleh:
                  row[colTahun] instanceof Date
                    ? row[colTahun].toISOString()
                    : safeString(row[colTahun]),
          ...
          for (let i = 1; i < jsonData.length; i++) {
            ...
            if (safeString(row[1]).toUpperCase().includes("DINAS PERKEBUNAN"))
              continue;
            const items = [];
            for (let k = 0; k < 6; k++) {
              ...
              if (
                bNama &&
                bNama !== "-" &&
                !bNama.toUpperCase().includes("DINAS PERKEBUNAN")
              ) {
                items.push({
                  nama: bNama.toUpperCase(),
                  merk: safeString(row[12 + k] || "-")
                    .trim()
                    .toUpperCase(),
            ...
            const payload = {
              nama: safeString(row[1] || "N/A")
                .trim()
                .toUpperCase(),
              nip: safeString(row[2]).trim(),
              skpd: safeString(row[5] || "")
                .trim()
                .toUpperCase(),
              jabatan: safeString(row[4] || "-")
                .trim()
                .toUpperCase(),
              bidang: safeString(row[5] || "-")
                .trim()
                .toUpperCase(),
            ...
            batch.set(
              doc(
                ...
                "data",
                "pegawai",
                safeString(row[2]).trim().replace(/\s/g, ""),
              ),
              payload,
              { merge: true },
            );
        ...
        if (document.getElementById("portalSync"))
          document.getElementById("portalSync").value = "";
        customAlert(
          "Berhasil",
          `Sinkronisasi selesai: ${count} baris data diproses.`,
        );
      } catch (e) {
        console.error("ERROR UPLOAD:", e);

        customAlert("Gagal", e.message);
      }
  ...
  const handleAddItem = async (form) => {
    if (!form.nama || !selectedUser || !user) return;
    setStatus({ ...status, loading: true });
    try {
      const formNoKartu = safeString(form.no_kartu).trim();
      const idToUse = formNoKartu
        ? formNoKartu.replace(/[^a-zA-Z0-9.-]/g, "_")
        : "MANUAL_" + Date.now();
      const isLocalOnly = form.saveToMaster === false; // Cek flag lokal
      const itemToSave = {
        nama: safeString(form.nama).toUpperCase(),
        merk: safeString(form.merk).toUpperCase(),
        tahun: safeString(form.tgl_pengadaan).split("-")[0],
        tgl_pengadaan: safeString(form.tgl_pengadaan),
        no_kartu: formNoKartu || idToUse,
        isLocalOnly,
      };

      const { masa_manfaat } = parseKodeBarangData(formNoKartu, kodeBarangData);
      const nilaiBukuVal = calculateNilaiBuku(
      ...
      if (!isLocalOnly) {
        await setDoc(
          doc(db, "artifacts", appId, "public", "data", "prices", idToUse),
          {
            unitId: activeUnitView,
            nama: safeString(form.nama).toUpperCase(),
            merk: safeString(form.merk).toUpperCase(),
            no_kartu: itemToSave.no_kartu,
            nilai_perolehan: parseFloat(form.nilai_perolehan) || 0,
            nilai_buku: nilaiBukuVal,
            tgl_pengadaan: safeString(form.tgl_pengadaan),
            tgl_habis: tglHabisVal,
            lokasi: selectedUser.nama,
            updatedAt: serverTimestamp(),
          },
          { merge: true },
        );
        await addDoc(
          ...
          {
            unitId: activeUnitView,
            itemName: safeString(form.nama),
            receiverName: selectedUser.nama,
            type: "INPUT BARU",
  ...
  const handleAddRoomItem = async (form) => {
    if (!form.nama || !selectedRoomName || !user) return;
    setStatus({ ...status, loading: true });
    try {
      const qty = parseInt(form.jumlah) || 1;
      const itemsToSave = [];
      const b = writeBatch(db);
      const timestamp = serverTimestamp();
      let baseNoKartu = safeString(form.no_kartu).trim();
      const roomDoc = filteredRuanganData.find(
        (r) => r.nama_ruangan === selectedRoomName,
      );
      const roomDocId =
        roomDoc && roomDoc.id
          ? roomDoc.id
          : `${activeUnitView}_${selectedRoomName.replace(/[^a-zA-Z0-9]/g, "_")}`;
      for (let i = 0; i < qty; i++) {
        let currentNoKartu = baseNoKartu;
        if (qty > 1 && baseNoKartu) {
          const match = baseNoKartu.match(/(.*?)(\d+)$/);
          if (match)
            currentNoKartu =
              match[1] +
              String(parseInt(match[2], 10) + i).padStart(match[2].length, "0");
          else currentNoKartu = `${baseNoKartu}-${i + 1}`;
        }
        const idToUse = currentNoKartu
          ? currentNoKartu.replace(/[^a-zA-Z0-9.-]/g, "_")
          : `RUANG_${Date.now()}_${i}`;
        ...
        const itemToSave = {
          ...
          tahun: safeString(form.tgl_pengadaan).split("-")[0],
          tgl_pengadaan: safeString(form.tgl_pengadaan),
          no_kartu: currentNoKartu || idToUse,
          isLocalOnly,
        };
      ...
      b.set(
        ...
        {
          unitId: activeUnitView,
          nama_ruangan: selectedRoomName,
          items: arrayUnion(...itemsToSave),
          updatedAt: timestamp,
        },
        { merge: true },
      );
      await b.commit();
      setModals({ ...modals, addRoomItem: false });
    } catch (e) {}
    setStatus({ ...status, loading: false });
  };
  const handleAddRoom = async () => {
    ...
    try {
      ...
      await setDoc(
        doc(db, "artifacts", appId, "public", "data", "ruangan", safeName),
        {
          unitId: activeUnitView,
          nama_ruangan: newRoomName,
          items: [],
          updatedAt: serverTimestamp(),
        },
        { merge: true },
      ...
      customAlert("Berhasil", "Ruangan ditambahkan.");
    } catch (e) {}
    setStatus({ ...status, loading: false });
  };
  const handleDeleteRoom = (roomName) => {
    const roomDoc = filteredRuanganData.find(
      (r) => r.nama_ruangan === roomName,
    );
    if (!roomDoc) return;
    customConfirm(
      "Hapus Ruangan?",
      `Yakin hapus ruangan "${roomName}"?`,
      "danger",
      async () => {
        setStatus({ ...status, loading: true });
        try {
          ...
          setSelectedRoomName(dynamicRooms[0] || "Default");
          customAlert("Berhasil", "Terhapus.");
        } catch (e) {}
        setStatus({ ...status, loading: false });
      },
    );
  ...
  const handleAddRoomName = async () => {
    if (!editRoom.newName || editRoom.newName === editRoom.oldName) return;
    setStatus({ ...status, loading: true });
    try {
      const roomDoc = (filteredRuanganData || []).find(
        (r) => r.nama_ruangan === editRoom.oldName,
      );
      const items = roomDoc ? roomDoc.items : [];
      const b = writeBatch(db);
      const newDocId = `${activeUnitView}_${editRoom.newName.replace(/[^a-zA-Z0-9]/g, "_")}`;
      b.set(
        doc(db, "artifacts", appId, "public", "data", "ruangan", newDocId),
        {
          unitId: activeUnitView,
          nama_ruangan: editRoom.newName,
          items,
          updatedAt: serverTimestamp(),
        },
        { merge: true },
      ...
      if (roomDoc && roomDoc.id) {
        b.delete(
          doc(db, "artifacts", appId, "public", "data", "ruangan", roomDoc.id),
        );
      }
      await b.commit();
      setSelectedRoomName(editRoom.newName);
      setEditRoom({ show: false, oldName: "", newName: "" });
      customAlert("Berhasil", "Nama diubah.");
    } catch (e) {}
    setStatus({ ...status, loading: false });
  };
    ...
    try {
      await addDoc(
        collection(db, "artifacts", appId, "public", "data", "rkbmd"),
        {
          unitId: activeUnitView,
          ...form,
          inputByRole: userRole,
          createdAt: serverTimestamp(),
        },
      );
      setModals({ ...modals, rkbmd: false });
    } catch (e) {}
    setStatus({ ...status, loading: false });
  };
  const deleteRkbmd = async (id) => {
    customConfirm(
      "Hapus RKBMD?",
      "Yakin hapus usulan ini?",
      "danger",
      async () => {
        setStatus({ ...status, loading: true });
        try {
          await setDoc(
            doc(db, "artifacts", appId, "public", "data", "rkbmd", id),
            { status: "deleted" },
            { merge: true },
          );
        } catch (e) {}
      ...
      async () => {
        ...
        try {
          if (type === "pegawai") {
            ...
            if (userToUpdate) {
              const newItems = userToUpdate.items.filter(
                (x) => !(x.no_kartu === item.no_kartu && x.nama === item.nama),
              );
              await updateDoc(
                doc(
                  db,
            ...
            const roomToUpdate = filteredRuanganData.find(
              (r) => r.nama_ruangan === selectedRoomName,
            );
            if (roomToUpdate && roomToUpdate.id) {
              const newItems = roomToUpdate.items.filter(
                (x) => !(x.no_kartu === item.no_kartu && x.nama === item.nama),
              );
              await updateDoc(
                doc(
          ...
          customAlert("Gagal", "Error hapus barang.");
        }
        setStatus({ ...status, loading: false });
      },
    );
  ...
  const handleUnlinkFromMaster = async (item, type) => {
    customConfirm(
      "Keluarkan dari Monitoring?",
      `Aset "${item.nama}" akan dihapus dari daftar Monitoring Aset (Master), namun akan TETAP DISIMPAN sebagai catatan lokal di sini. Lanjutkan?`,
      "danger",
      async () => {
        setStatus({ ...status, loading: true });
        try {
          ...
          b.set(
            ...
            ),
            {
              ...
              senderName: "Sistem",
              receiverName:
                type === "ruangan" ? selectedRoomName : selectedUser.nama,
              type: "UBAH KE LOKAL",
              timestamp: serverTimestamp(),
          ...
          customAlert(
            "Berhasil",
            "Aset berhasil dikeluarkan dari Monitoring dan sekarang menjadi Catatan Lokal.",
          );
        } catch (e) {
          console.error(e);
          customAlert(
            "Gagal",
            "Terjadi kesalahan saat memproses pemindahan data.",
          );
        }
        setStatus({ ...status, loading: false });
      },
  ...
  const processTransfer = async (targetNipParam) => {
    ...
    try {
      ...
      if (selectedUser) {
        ...
        if (senderUser) {
          const newItems = senderUser.items.filter(
            (x) =>
              !(
                x.no_kartu === safeTransferItem.no_kartu &&
                x.nama === safeTransferItem.nama
              ),
          );
          b.update(
            doc(
      ...
      b.update(
        doc(
          ...
          "data",
          "pegawai",
          safeString(target.nip).replace(/\s/g, ""),
        ),
        { items: arrayUnion(safeTransferItem) },
      );
      b.set(
        doc(collection(db, "artifacts", appId, "public", "data", "histori")),
        {
          unitId: activeUnitView,
          itemName: safeString(safeTransferItem.nama),
          senderName: safeString(selectedUser.nama),
          receiverName: safeString(target.nama),
          timestamp: serverTimestamp(),
          type: "MUTASI",
        },
      );

      const safeId = safeTransferItem.no_kartu
        ? safeString(safeTransferItem.no_kartu).replace(/[^a-zA-Z0-9.-]/g, "_")
        : null;
      if (safeId && !safeTransferItem.isLocalOnly)
        b.set(
          doc(db, "artifacts", appId, "public", "data", "prices", safeId),
          { lokasi: target.nama, updatedAt: serverTimestamp() },
          { merge: true },
        );

      await b.commit();
      setModals({ ...modals, transfer: false });
      await generateBAST(safeTransferItem, selectedUser, target);
    } catch (e) {
      customAlert("Gagal", "Error mutasi.");
    }
    setStatus({ ...status, loading: false });
  };
  const processTransferUnit = async (targetUnit, targetBidang) => {
    ...
    try {
      ...
      if (sourceType === "pegawai") {
        ...
        if (senderPegawai) {
          const newItems = senderPegawai.items.filter(
            (x) => !(x.no_kartu === item.no_kartu && x.nama === item.nama),
          );
          b.update(
            doc(
              ...
              "data",
              "pegawai",
              safeString(sourceRef).replace(/\s/g, ""),
            ),
            { items: newItems },
          );
        }
        ...
        const roomDoc = filteredRuanganData.find(
          (r) => r.nama_ruangan === sourceRef,
        );
        if (roomDoc && roomDoc.id) {
          const newItems = roomDoc.items.filter(
            (x) => !(x.no_kartu === item.no_kartu && x.nama === item.nama),
          );
          b.update(
            doc(
      ...
      b.set(
        doc(collection(db, "artifacts", appId, "public", "data", "histori")),
        {
          unitId: activeUnitView,
          itemName: safeString(item.nama),
          senderName: UNIT_LABELS[activeUnitView],
          receiverName: UNIT_LABELS[targetUnit] + " (Menunggu Persetujuan)",
          timestamp: serverTimestamp(),
          type: "PENGAJUAN MUTASI ANTAR UNIT",
        },
      );
      ...
      const safeId = item.no_kartu
        ...
        : null;
      if (safeId && !item.isLocalOnly)
        b.set(
          doc(db, "artifacts", appId, "public", "data", "prices", safeId),
          {
            lokasi: `Menunggu Persetujuan Mutasi ke ${UNIT_LABELS[targetUnit]}`,
            updatedAt: serverTimestamp(),
          },
          { merge: true },
        );
      ...
      generateBASTAntarUnit(
        safeTransferItem,
        activeUnitView,
        targetUnit,
        senderPegawai,
      );
      setTransferUnitModal({
        show: false,
        item: null,
        sourceRef: null,
        sourceType: "",
      ...
      customAlert(
        "Berhasil",
        `Pengajuan mutasi aset ke ${UNIT_LABELS[targetUnit]} telah dikirim dan menunggu persetujuan admin tujuan.`,
      );
    } catch (e) {
      console.error(e);
  ...
  const handleApproveMutasi = async (mutasi) => {
    ...
    try {
      const b = writeBatch(db);
      const safeId = mutasi.item.no_kartu
        ? safeString(mutasi.item.no_kartu).replace(/[^a-zA-Z0-9.-]/g, "_")
        : null;
      const newLokasi =
        mutasi.targetUnit === UNIT_CODES.PUSAT
          ? `Transit - ${mutasi.targetBidang}`
          : `Transit ${UNIT_LABELS[mutasi.targetUnit]}`;
      if (safeId) {
        const assetPayload = {
      ...
      customAlert(
        "Berhasil",
        "Mutasi diterima. Aset sekarang berada di " +
          newLokasi +
          " pada Master Data.",
      );
    } catch (e) {
  ...
  const handleRejectMutasi = async (mutasi) => {
    ...
    try {
      ...
      if (mutasi.sourceType === "pegawai") {
        b.update(
          doc(
            ...
            "data",
            "pegawai",
            safeString(mutasi.sourceRef).replace(/\s/g, ""),
          ),
          { items: arrayUnion(mutasi.item) },
        );
      } else if (mutasi.sourceType === "ruangan") {
        const roomDoc = ruanganData.find(
          (r) =>
            r.nama_ruangan === mutasi.sourceRef &&
            (r.unitId || UNIT_CODES.PUSAT) === mutasi.sourceUnit,
        );
        const roomDocId =
          roomDoc && roomDoc.id
            ? roomDoc.id
            : `${mutasi.sourceUnit}_${mutasi.sourceRef.replace(/[^a-zA-Z0-9]/g, "_")}`;
        b.set(
          doc(db, "artifacts", appId, "public", "data", "ruangan", roomDocId),
          {
            unitId: mutasi.sourceUnit,
            nama_ruangan: mutasi.sourceRef,
            items: arrayUnion(mutasi.item),
            updatedAt: serverTimestamp(),
          },
          { merge: true },
      ...
      customAlert(
        "Berhasil",
        "Mutasi ditolak dan aset dikembalikan ke " + mutasi.senderName,
      );
    } catch (e) {
      customAlert("Gagal", "Terjadi kesalahan.");
    }
  ...
  const handleRestoreItem = async (item) => {
    ...
    const msg = isManual
      ? "Aset akan dikembalikan ke Monitoring Aset sesuai status aslinya."
      : "Aksi ini menahan status aset tetap Aktif.";
  ...
  const handleDeleteVehicle = async (item) => {
    customConfirm(
      "Hapus Kendaraan",
      "Kendaraan akan dipindahkan ke daftar Penghapusan dan dapat dipulihkan kembali.",
      "danger",
      async () => {
        setStatus({ ...status, loading: true });
        ...
        try {
          ...
          customAlert(
            "Berhasil",
            "Kendaraan berhasil dipindahkan ke Penghapusan."
          );
        } catch (e) {
  ...
  const processManualDisposal = async (item, alasan) => {
    ...
    try {
      ...
      await setDoc(
        ...
        {
          manual_disposal: true,
          alasan_hapus: safeString(alasan),
          updatedAt: serverTimestamp(),
        },
        { merge: true },
      );
      customAlert(
        "Berhasil",
        "Aset berhasil dipindahkan ke usulan penghapusan.",
      );
    ...
    try {
      ...
      if (item.pemegang && item.pemegang !== "-") {
        ...
        if (pUser) {
          const newItems = pUser.items.filter(
            (x) => x.no_kartu !== item.no_kartu,
          );
          b.update(
            doc(
              db,
              ...
              "data",
              "pegawai",
              safeString(pUser.nip).replace(/\s/g, ""),
            ),
            { items: newItems },
          );
        }
      ...
      // Atau hapus item dari daftar Ruangan (jika ada)
        ...
        const rData = (ruanganData || []).find(
          (r) =>
            ...
            (r.unitId || UNIT_CODES.PUSAT) === (item.unitId || activeUnitView),
        );
        if (rData && rData.id) {
          const newItems = rData.items.filter(
            (x) => x.no_kartu !== item.no_kartu,
          );
          b.update(
            doc(db, "artifacts", appId, "public", "data", "ruangan", rData.id),
            { items: newItems },
          );
        }
      }
      ...
      b.set(
        doc(collection(db, "artifacts", appId, "public", "data", "histori")),
        {
          unitId: activeUnitView,
          itemName: safeString(item.nama),
          senderName: `Sistem (Dihapus oleh ${editorName})`,
          receiverName: "Dihapus Permanen",
          type: "PENGHAPUSAN PERMANEN",
          timestamp: serverTimestamp(),
          no_kartu: item.no_kartu,
          alasan: item.alasan_hapus,
        },
      );
  ...
  const handlePerpanjangPajak = async (item) => {
    customConfirm(
      "Perpanjang Pajak",
      `Tambah masa aktif pajak 1 tahun untuk ${item.nama}?`,
      "info",
      async () => {
        setStatus({ ...status, loading: true });
        try {
          let d = new Date(item.tgl_pajak || item.tgl_pengadaan || Date.now());
          ...
          await setDoc(
            doc(db, "artifacts", appId, "public", "data", "prices", safeId),
            { tgl_pajak: d.toISOString(), updatedAt: serverTimestamp() },
            { merge: true },
          );
          customAlert("Berhasil", "Pajak diperpanjang.");
        } catch (e) {}
        setStatus({ ...status, loading: false });
      },
    );
  ...
  const processPajakOwner = async (targetIdParam) => {
    const { item } = editPajakModal;
    console.log("processPajakOwner dijalankan");
    if (!item || !user) return;
    setStatus({ ...status, loading: true });
    try {
      const safeId = item.no_kartu
...
const processSaveVehicle = async (vehicleData) => {
  ...
  const processEditNilai = async (
    ...
    if (pass !== "SugengR" && pass !== "AchmarA")
      return customAlert("Gagal", "Password salah!");
    const { item } = editNilaiModal;
    if (!item || !user) return;
    setStatus({ ...status, loading: true });
    try {
      ...
      await setDoc(
        ...
        ),
        {
          ...
          tgl_pengadaan: tglPengadaanBaru
            ? new Date(tglPengadaanBaru).toISOString()
            : item.tgl_pengadaan,
          tgl_habis: tglHabisBaru
            ? new Date(tglHabisBaru).toISOString()
            : item.tgl_habis,
          updatedAt: serverTimestamp(),
        },
        { merge: true },
      );
      customAlert("Berhasil", "Nilai pengadaan dan masa pakai telah diupdate.");
      setEditNilaiModal({ show: false, item: null });
    } catch (e) {}
    setStatus({ ...status, loading: false });
  };
  const handleSaveFeedback = async (rating, comment) => {
    ...
    try {
      ...
      customAlert(
        "Terima Kasih!",
        "Ulasan dan masukan Anda telah terkirim. Kami sangat menghargainya.",
      );
  ...
  const uploadToCloudinary = async (file) => {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        errBody?.error?.message || `Upload gagal (status ${res.status})`,
      );
    }
  ...
  const savePhotoToCloud = async () => {
    ...
    try {
      ...
      // ==========================
        ...
        const tglPajak = new Date(today);
        tglPajak.setFullYear(tglPajak.getFullYear() + 1);

        const tglPajak5 = new Date(today);
        tglPajak5.setFullYear(tglPajak5.getFullYear() + 5);
        ...
        await setDoc(
          doc(db, "artifacts", appId, "public", "data", "prices", safeId),
          {
            ...
            tgl_pajak: tglPajak.toISOString(),

            tgl_pajak_5_tahun: tglPajak5.toISOString(),
      ...
      // ==========================
        ...
        const safeId =
          photoModal.item.id ||
          safeString(photoModal.item.kib).replace(/[^a-zA-Z0-9.-]/g, "_");
      ...
      // ==========================
        ...
        await setDoc(
          doc(db, "artifacts", appId, "public", "data", "rkbmd_arsip", safeId),
          {
            fileUrl: downloadURL,
            updatedAt: serverTimestamp(),
          },
      ...
      reader.onload = (ev) => {
        ...
        img.onload = () => {
          ...
          setPhotoModal((p) => ({
            ...p,
            file: file,
            preview: c.toDataURL("image/jpeg", 0.6),
            isPdf: false,
            isExcel: false,
            fileName: file.name,
          }));
  ...
  const uploadMediaTanahToCloud = async () => {
    ...
    try {
      ...
      customAlert(
        "Upload Gagal",
        `Pesan Error: ${err.message}. Pastikan Firebase Storage Rules & konfigurasi CORS sudah diatur.`,
      );
  ...
  const handleUpdateTanahInfo = async (form, item) => {
    ...
    if (form.password === "SugengR") editorName = "Sugeng Riyanto";
      ...
      customAlert("Gagal", "Password salah.");
      return;
    }
    setStatus({ ...status, loading: true });
    try {
      ...
      await setDoc(
        doc(db, "artifacts", appId, "public", "data", "tanah", safeId),
        {
          kib: safeString(form.kib),
          deskripsi: safeString(form.deskripsi),
          tgl_peroleh: safeString(form.tgl_peroleh),
          luas: safeString(form.luas),
          alamat: safeString(form.alamat),
          status_hak: safeString(form.status_hak),
          penggunaan: safeString(form.penggunaan),
          petugas: safeString(form.petugas),
          tlp_petugas: safeString(form.tlp),
          gmaps: safeString(form.gmaps),
          biaya_pengurusan: safeString(form.biaya_pengurusan),
          penerimaan_pad: safeString(form.penerimaan_pad),
          tarif_retribusi: safeString(form.tarif_retribusi),
          total_tarif: safeString(form.total_tarif),
          satuan_tarif: safeString(form.satuan_tarif),
      ...
      customAlert("Gagal", "Kesalahan saat menyimpan.");
    }
    setStatus({ ...status, loading: false });
  };
  ...
  const handleUploadRkbmdTemplate = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      setRkbmdTemplateHeaders(headers);
      customAlert("Template Aktif", "Format Excel dimuat.");
    };
  ...
  const handleExportSuratJalan = async () => {
    if (
      !suratJalanForm.kendaraanId ||
      !suratJalanForm.tujuan ||
      !suratJalanForm.pengemudi ||
      !suratJalanForm.pengurusId
    ) {
      return customAlert(
        "Gagal",
        "Lengkapi data pengemudi, kendaraan, tujuan, dan pengurus barang.",
      );
    }
    const kendaraan = vehicleItems.find(
      (v) => v.no_kartu === suratJalanForm.kendaraanId,
    );
    if (!kendaraan) return;
    const kasubbag = (filteredPegawaiData || []).find((p) =>
      safeString(p.nama).toLowerCase().includes("indira dwigus"),
    ) || {
      nama: "INDIRA DWIGUS KIRANA, S.E., M.A.",
      nip: "19830825 201001 2 015",
    };
    const pengurus = pengurusOptions.find(
      (p) => p.nip === suratJalanForm.pengurusId,
    ) || { nama: "Pengurus Barang", nip: suratJalanForm.pengurusId };
    const tglCetak = new Date(suratJalanForm.tglBerangkat).toLocaleDateString(
      "id-ID",
    ...
    if (window.saveAs)
      window.saveAs(
        blob,
        `Surat_Jalan_${safeString(kendaraan.no_kartu).replace(/[^a-zA-Z0-9.-]/g, "_")}.doc`,
      );
    else {
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      a.download = `Surat_Jalan_${safeString(kendaraan.no_kartu).replace(/[^a-zA-Z0-9.-]/g, "_")}.doc`;
      a.click();
    }
    setStatus({ ...status, loading: true });
    try {
      ...
      customAlert(
        "Berhasil",
        "Surat jalan berhasil dicetak dan dicatat di riwayat pemakaian.",
      );
    } catch (e) {
      console.error(e);
    }
  ...
  const handleExportTanah = () => {
    ...
    const rows = filteredTanahDataForView.map((t) => ({
      "KIB / Nomor": safeString(t.kib),
      "Deskripsi / Jenis": safeString(t.deskripsi),
      "Tgl Peroleh": getSafeDateString(t.tgl_peroleh).split("T")[0],
      "Luas (m2)": safeString(t.luas),
      "Alamat Lengkap": safeString(t.alamat),
      "Status Hak": safeString(t.status_hak),
      Penggunaan: safeString(t.penggunaan),
      "Petugas PIC": safeString(t.petugas),
      Telp: safeString(t.tlp_petugas),
      "Biaya Pengurusan/Thn": safeString(t.biaya_pengurusan),
      "Penerimaan PAD/Thn": safeString(t.penerimaan_pad),
      "Tarif Retribusi": safeString(t.tarif_retribusi),
      "Satuan Tarif": safeString(t.satuan_tarif),
      "Total Tarif Jika Disewakan": safeString(t.total_tarif),
    }));
    const ws = window.XLSX.utils.json_to_sheet(rows);
    const wb = window.XLSX.utils.book_new();
    window.XLSX.utils.book_append_sheet(wb, ws, "Tanah");
    window.XLSX.writeFile(
      wb,
      `Inventaris_Tanah_${UNIT_LABELS[activeUnitView].replace(/ /g, "_")}.xlsx`,
    );
  };
  ...
  const handleExportSPPKD = () => {
    customAlert("Dalam Pengembangan", "Export SPPKD belum diimplementasikan.");
  };
  ...
  const handleExportWordSPPBI = async (single) => {
    try {
      ...
      if (targets.length === 0) {
        return customAlert("Kosong", "Data kosong.");
      }
      ...
      for (const p of targets) {
        ...
        userItemsToShow = userItemsToShow.filter(
          (it) =>
            !safeString(it.nama).toUpperCase().includes("DINAS PERKEBUNAN"),
        );
  ...
  const handleExportKIRWord = (roomName) => {
    const roomData = (filteredRuanganData || []).find(
      (r) => r.nama_ruangan === roomName,
    );
    ...
    roomData.items.forEach((it) => {
      const key = `${it.nama}_${it.merk}`;
      if (!groupedItems[key]) groupedItems[key] = { ...it, volume: 0 };
      groupedItems[key].volume += 1;
    });
    ...
    ) || { nama: "INDIRA DWIGUS KIRANA", nip: "............................" };
    const pengurus = (filteredPegawaiData || []).find(
      (p) =>
        p.nama && safeString(p.nama).toLowerCase().includes("sugeng riyanto"),
    ) || { nama: "SUGENG RIYANTO", nip: "............................" };
    const tglCetak = new Date().toLocaleDateString("id-ID", {
      day: "numeric",
      month: "long",
      year: "numeric",
    });
    let tableRows = itemsToPrint
      .map(
        (it, idx) =>
          `<tr><td class="text-center">${idx + 1}</td><td>${safeString(it.nama)}</td><td>${safeString(it.merk || "-")}</td><td class="text-center">${safeString(it.volume)}</td><td></td><td></td></tr>`,
      )
      .join("");
    let html = "";
    if (templates.kir) {
      html = templates.kir
        .replace(/\{\{ROOM_NAME\}\}/g, safeString(roomName))
        .replace(/\{\{KASUBAG_NAMA\}\}/g, safeString(kasubag.nama))
        .replace(/\{\{KASUBAG_NIP\}\}/g, safeString(kasubag.nip))
        .replace(/\{\{PENGURUS_NAMA\}\}/g, safeString(pengurus.nama))
        .replace(/\{\{PENGURUS_NIP\}\}/g, safeString(pengurus.nip))
        .replace(/\{\{TABLE_ROWS\}\}/g, tableRows)
        .replace(/\{\{TANGGAL\}\}/g, tglCetak);
    } else {
      html = `<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'><head><meta charset='utf-8'><style>body{font-family:'Arial', sans-serif; font-size: 11pt;} .header-table{width: 100%; border-collapse: collapse; margin-bottom: 20px;} .header-table th, .header-table td{border: 1px solid black; padding: 5px; text-align: left; vertical-align: middle;} .text-center{text-align: center;} .bold{font-weight: bold;} .main-table{width: 100%; border-collapse: collapse;} .main-table th, .main-table td{border: 1px solid black; padding: 5px;} .title{font-size: 14pt; font-weight: bold; text-align: center; margin: 20px 0; text-transform: uppercase;}</style></head><body><table class="header-table"><tr><td rowspan="4" width="15%" class="text-center"><b>[LOGO]</b></td><td rowspan="1" colspan="2" class="text-center bold">DINAS PERKEBUNAN PROVINSI JAWA TIMUR</td><td width="15%">No. Dokumen</td><td width="20%">: </td></tr><tr><td rowspan="3" width="15%" class="text-center bold">FORM</td><td rowspan="3" class="text-center bold">Kartu Inventaris Ruangan</td><td>Terbitan/Revisi</td><td>: </td></tr><tr><td>Tanggal Terbit</td><td>: </td></tr><tr><td>Halaman</td><td>: -</td></tr></table><div class="title">${safeString(roomName)}</div><table class="main-table"><thead><tr><th rowspan="2" width="5%" class="text-center">No</th><th rowspan="2" width="30%" class="text-center">Nama Barang/Jasa</th><th rowspan="2" width="35%" class="text-center">Spesifikasi Teknis</th><th rowspan="2" width="10%" class="text-center">Volume</th><th colspan="2" width="20%" class="text-center">Kondisi</th></tr><tr><th class="text-center">Layak</th><th class="text-center">Tidak Layak</th></tr></thead><tbody>${tableRows}${Array.from(
        { length: Math.max(0, 15 - itemsToPrint.length) },
      )
        .map(
          (_, i) =>
            `<tr><td class="text-center">${itemsToPrint.length + i + 1}</td><td>&nbsp;</td><td></td><td></td><td></td><td></td></tr>`,
        )
        .join(
          "",
    ...
    const blob = new Blob(["\ufeff", html], { type: "application/msword" });
    if (window.saveAs) window.saveAs(blob, `KIR.doc`);
    else {
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      a.download = `KIR.doc`;
  ...
  const handleExportExcel = (type) => {
    if (!window.XLSX) return customAlert("Tunggu", "Loading Excel...");
    const rows = monitoringDataFiltered.map((item) => ({
      _active: item._active,
      _disposal: item._disposal,
      "No Kartu": item.no_kartu || "-",
      "Nama Barang": item.nama,
      "Lokasi/Pemegang":
        item.ruangan !== "-"
          ? `Ruang: ${item.ruangan}`
          : item.pemegang !== "-"
            ? `Pegawai: ${item.pemegang}`
            : item.lokasi || "-",
      "Tgl Pengadaan": formatDisplayDate(getSafeDateString(item.tgl_pengadaan)),
      "Nilai Perolehan": item.nilai_perolehan,
      "Nilai Buku": item._active ? item.nilai_buku : 0,
      "Tgl Expired": formatDisplayDate(getSafeDateString(item.tgl_habis)),
      Status: item.manual_active
        ? "DIPERPANJANG"
        : item.isExpired
          ? "HABIS"
          : "AKTIF",
      Foto: item.photoBase64 ? "ADA" : "TIDAK",
    }));
    let final = [];
    if (type === "all") final = rows;
    if (type === "active") final = rows.filter((r) => r._active);
    if (type === "disposal") final = rows.filter((r) => r._disposal);
    if (final.length === 0) return customAlert("Kosong", "Data kosong.");
    const ws = window.XLSX.utils.json_to_sheet(
      final.map(({ _active, _disposal, ...r }) => r),
    );
    const wb = window.XLSX.utils.book_new();
    window.XLSX.utils.book_append_sheet(wb, ws, `Data Aset`);
    window.XLSX.writeFile(
      wb,
      `Laporan_${type}_${UNIT_LABELS[activeUnitView].replace(/ /g, "_")}.xlsx`,
    );
  };
  const handleExportRkbmd = (bidang) => {
    if (displayedRkbmdData.length === 0)
      return customAlert("Kosong", "Data kosong.");
    ...
    let tableRows = displayedRkbmdData
      .map(
        (r, i) => `
          <tr>
  ...
  const handleExportPajak = () => {
    ...
    if (vehicleItems.length === 0)
      return customAlert("Kosong", "Data kendaraan kosong.");
    ...
    window.XLSX.writeFile(
      wb,
      `Laporan_Pajak_Kendaraan_${UNIT_LABELS[activeUnitView].replace(/ /g, "_")}.xlsx`,
    );
  };
  const handleExportWordSPPBIVehicle = (item) => {
    const locStr =
      item.pemegang !== "-"
        ...
        : item.ruangan !== "-"
          ? item.ruangan
          : item.lokasi;
    const ownerPegawai = (data || []).find(
      (p) =>
        safeString(p.nama).toLowerCase() === safeString(locStr).toLowerCase(),
    ) || {
      nama: locStr,
      nip: "............................",
      jabatan: "............................",
    ...
    if (templates.sppbi) {
      html += templates.sppbi
        .replace(/\{\{NAMA\}\}/g, safeString(ownerPegawai.nama))
        .replace(/\{\{NIP\}\}/g, safeString(ownerPegawai.nip))
        .replace(/\{\{JABATAN\}\}/g, safeString(ownerPegawai.jabatan))
        .replace(/\{\{TABLE_ROWS\}\}/g, tableRows)
    ...
    if (window.saveAs)
      window.saveAs(
        blob,
        `SPPBI_Kendaraan_${safeString(item.no_kartu).replace(/[^a-zA-Z0-9.-]/g, "_")}.doc`,
      );
    else {
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      a.download = `SPPBI_Kendaraan_${safeString(item.no_kartu).replace(/[^a-zA-Z0-9.-]/g, "_")}.doc`;
      a.click();
    }
  };
  const handleExportRekapPengguna = () => {
    ...
    const getYear = (it) => {
      const y =
        parseDateRobust(getSafeDateString(it.tgl_pengadaan))
          ?.getFullYear()
          ?.toString() || safeString(it.tgl_pengadaan || it.tahun || "");
      return y.length > 4 ? y.substring(0, 4) : y;
    };
    let filteredData = activeMonitoringData;
    if (monitoringSelectedYear !== "Semua") {
      filteredData = filteredData.filter(
        (it) => getYear(it) === monitoringSelectedYear,
      );
    }
    if (filteredData.length === 0)
      return customAlert("Kosong", "Data kosong pada filter tahun tersebut.");
    const asetKendaraan = [];
    const asetBarang = [];
    filteredData.forEach((it) => {
      ...
      const baseObj = {
        ...
        KONDISI: "BAIK",
        "PENGGUNA / LOKASI":
          it.ruangan !== "-" ? safeString(it.ruangan) : safeString(it.pemegang),
      };
      if (isVehicle || it.tgl_pajak) {
        asetKendaraan.push({
          ...baseObj,
          "NOMOR POLISI": safeString(it.no_kartu),
          "TGL PAJAK": formatDisplayDate(
    ...
    const wb = window.XLSX.utils.book_new();
    if (finalBarang.length > 0)
      window.XLSX.utils.book_append_sheet(
        wb,
        window.XLSX.utils.json_to_sheet(finalBarang),
        "Rekap Barang Inventaris",
      );
    if (finalKendaraan.length > 0)
      window.XLSX.utils.book_append_sheet(
        wb,
        window.XLSX.utils.json_to_sheet(finalKendaraan),
        "Rekap Kendaraan Dinas",
      );
    const yearLabel =
      monitoringSelectedYear !== "Semua"
        ? monitoringSelectedYear
        : "Semua_Tahun";
    window.XLSX.writeFile(
      wb,
      `Rekap_Aset_${yearLabel}_${UNIT_LABELS[activeUnitView].replace(/ /g, "_")}.xlsx`,
    );
  };
  const menuItems = [
    ...
    },
    {
      ...
      show:
        (userRole === "admin" && activeUnitView === UNIT_CODES.PUSAT) ||
        userRole === "staff",
    },
    ...
    { id: "monitoring", icon: Search, label: "Monitoring Aset", show: true },
    {
      ...
      show: userRole === "admin" || userRole === "admin_upt",
    },
    {
      id: "laporan",
      icon: FileDown,
  ...
  if (!isLoggedIn) {
    return (
      <div
        className={`min-h-screen flex items-center justify-center p-4 md:p-8 font-sans transition-colors duration-300 ${isDarkMode ? "dark bg-slate-900 text-white" : "bg-slate-100"}`}
      >
        <div
          className={`w-full max-w-5xl rounded-[2rem] md:rounded-[3rem] shadow-2xl flex flex-col md:flex-row overflow-hidden min-h-[600px] ${isDarkMode ? "bg-slate-800" : "bg-white"}`}
        >
          <div className="w-full md:w-1/2 bg-emerald-700 p-8 md:p-12 text-white flex flex-col justify-center relative">
            <div className="relative z-10">
              <div className="flex items-center mb-6 text-white">
                <Globe size={48} />
                <Leaf size={40} className="-ml-3 mt-4 text-emerald-300" />
              </div>
              ...
              <p className="opacity-80">Sistem Manajemen Aset Terintegrasi</p>
            </div>
          </div>
          <div className="w-full md:w-1/2 p-8 md:p-16 flex flex-col justify-center relative">
            <button
              onClick={() => setIsDarkMode(!isDarkMode)}
              className="absolute top-6 right-6 p-2 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300 hover:bg-emerald-100"
            >
              {isDarkMode ? <Sun size={20} /> : <Moon size={20} />}
            </button>
            <h2
              className={`text-3xl font-black mb-8 ${isDarkMode ? "text-white" : "text-slate-800"}`}
            >
              Login Portal
            </h2>
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="space-y-2">
                ...
                <input
                  type="text"
                  className={`w-full p-4 rounded-xl border outline-none focus:border-emerald-500 ${isDarkMode ? "bg-slate-700 border-slate-600 text-white" : "bg-slate-50"}`}
              ...
              <button
                type="submit"
                className="w-full py-4 bg-emerald-600 text-white rounded-xl font-bold uppercase hover:bg-emerald-500 transition-all shadow-lg"
              >
                Masuk
              </button>
            </form>
  ...
  return (
    <div
      className={`min-h-screen flex font-sans overflow-hidden relative transition-colors duration-300 ${isDarkMode ? "dark bg-slate-900 text-white" : "bg-[#FDFDFD] text-slate-900"}`}
    >
      {isMobileMenuOpen && (
        <div
      ...
      <aside
        className={`w-80 border-r flex flex-col p-8 fixed h-full z-40 transition-transform transform ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 ${isDarkMode ? "bg-slate-800 border-slate-700" : "bg-white"}`}
      >
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-emerald-600 rounded-2xl text-white flex items-center">
              <Globe size={24} />
              <Leaf size={20} className="-ml-2 mt-2 text-emerald-300" />
            </div>
            <div>
              <h1 className="font-black text-xl">INVENSBUN</h1>
              <p className="text-[10px] font-black text-emerald-500 tracking-widest">
                {safeString(userRole).toUpperCase()}
              </p>
            </div>
          </div>
          <button
          ...
          {menuItems
            ...
            .map((m) => (
              <button
                ...
                className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-bold transition-all ${activeTab === m.id ? "bg-emerald-600 text-white shadow-lg" : isDarkMode ? "text-slate-400 hover:bg-slate-700" : "text-slate-400 hover:bg-slate-50"}`}
              >
        ...
        <div
          className={`mt-4 pt-4 border-t space-y-3 ${isDarkMode ? "border-slate-700" : ""}`}
        >
          <button
            onClick={() => setModals({ ...modals, feedback: true })}
            className={`w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-xs font-bold transition-colors ${isDarkMode ? "bg-yellow-900/30 text-yellow-400 hover:bg-yellow-900/50" : "bg-yellow-50 text-yellow-600 hover:bg-yellow-100"}`}
          >
            <MessageSquare size={16} /> Beri Masukan
          </button>
          <div className="flex justify-center mb-4">
            <button
              onClick={() => setIsDarkMode(!isDarkMode)}
              className={`flex w-full justify-center items-center gap-2 px-4 py-3 rounded-xl text-xs font-bold transition-colors ${isDarkMode ? "bg-slate-700 text-yellow-400 hover:bg-slate-600" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
            >
          ...
          {!isReadOnly && (
            <button
              onClick={() => {
                setStatus({
                  ...status,
                  uploadType: userRole === "admin" ? "pegawai" : "prices",
                });
                setModals({ ...modals, upload: true });
              }}
              className="w-full py-4 bg-emerald-600 text-white rounded-xl font-bold text-xs uppercase flex items-center justify-center gap-2 hover:bg-emerald-700"
            >
              <CloudUpload size={16} /> Sync Data DB
            </button>
          )}
          <button
            onClick={() => setIsLoggedIn(false)}
            className="w-full py-3 text-red-400 hover:text-red-600 font-bold text-xs uppercase flex items-center justify-center gap-2"
          >
            <Lock size={14} /> Logout
          </button>
        </div>
      ...
      <main
        className={`flex-grow w-full md:ml-80 p-6 md:p-12 min-h-screen overflow-y-auto pb-32`}
      >
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-3xl md:text-4xl font-black capitalize">
                {safeString(activeTab).replace("-", " ")}
              </h2>
              <p className="text-slate-400 font-bold mt-1 hidden md:block">
                Portal Aset & Inventaris Terintegrasi
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 md:gap-4">
            {userRole === "admin" && (
              <div
                className={`flex items-center gap-2 px-4 py-2 rounded-2xl border shadow-sm ${isDarkMode ? "bg-slate-800 border-slate-700" : "bg-white"}`}
              >
                <Building className="text-emerald-500" size={16} />
                <select
                  value={activeUnitView}
                  onChange={(e) => setActiveUnitView(e.target.value)}
                  className="bg-transparent font-bold text-xs md:text-sm outline-none dark:text-white cursor-pointer"
                >
            ...
            {userRole === "admin_upt" && (
              <div
                className={`px-4 py-2 md:px-6 md:py-3 rounded-2xl border shadow-sm flex items-center gap-2 font-bold text-xs md:text-sm text-emerald-600 ${isDarkMode ? "bg-slate-800 border-slate-700" : "bg-white"}`}
              >
                <Building size={16} /> {UNIT_LABELS[userUnit]}
              </div>
            )}
            <div
              className={`px-4 py-2 md:px-6 md:py-3 rounded-2xl border shadow-sm flex items-center gap-3 ${isDarkMode ? "bg-slate-800 border-slate-700" : "bg-white"}`}
            >
              <div
                className={`w-3 h-3 rounded-full ${status.synced ? "bg-emerald-500 animate-pulse" : "bg-orange-500"}`}
              ></div>
              <span className="text-[10px] md:text-xs font-bold uppercase hidden md:inline">
                {status.synced ? "Online" : "Syncing..."}
              </span>
            </div>
          </div>
        </header>
        ...
        {activeTab === "dashboard" && (
          <DashboardView
            ...
            filteredPegawaiData={filteredPegawaiData}
            filteredPriceData={filteredPriceData}
            handleApproveMutasi={handleApproveMutasi}
            handleRejectMutasi={handleRejectMutasi}
            isDarkMode={isDarkMode}
            mutasiPendingData={mutasiPendingData}
            setActiveTab={setActiveTab}
            setShowDashboardAlert={setShowDashboardAlert}
            showDashboardAlert={showDashboardAlert}
            statsAsetAktif={statsAsetAktif}
            statsPenghapusan={statsPenghapusan}
            statsReAktif={statsReAktif}
            status={status}
        ...
        {activeTab === "data" && (
          <DataAsetView
            ...
            data={data}
            handleExportBulkLabel={handleExportBulkLabel}
            handleExportWordSPPBI={handleExportWordSPPBI}
            handleSearchChange={handleSearchChange}
            isDarkMode={isDarkMode}
            isPegawaiEditable={isPegawaiEditable}
            modals={modals}
            priceData={priceData}
            ...
            setTransferUnitModal={setTransferUnitModal}
            templates={templates}
          />
        )}
        {/* --- Inventaris Ruangan --- */}
        {activeTab === "ruangan" && (
          <RuanganView
            ...
            handleExportBulkLabel={handleExportBulkLabel}
            handleExportKIRWord={handleExportKIRWord}
            handleUnlinkFromMaster={handleUnlinkFromMaster}
            isDarkMode={isDarkMode}
            isPegawaiEditable={isPegawaiEditable}
        ...
        {activeTab === "pajak" && (
          <PajakView
            handleExportPajak={handleExportPajak}
            handleExportWordSPPBIVehicle={handleExportWordSPPBIVehicle}
            ...
            pajakYearsList={pajakYearsList}
            setEditPajakModal={setEditPajakModal}
            setPajakSelectedYear={setPajakSelectedYear}
            setPajakYearsList={setPajakYearsList}
            setPhotoModal={setPhotoModal}
        ...
        {activeTab === "surat-kendaraan" &&
          ((userRole === "admin" && activeUnitView === UNIT_CODES.PUSAT) ||
            userRole === "staff") && (
            <SuratKendaraanView
              availableVehicles={availableVehicles}
              handleExportSuratJalan={handleExportSuratJalan}
              isDarkMode={isDarkMode}
              pengurusOptions={pengurusOptions}
              setSuratJalanForm={setSuratJalanForm}
              suratJalanForm={suratJalanForm}
              suratJalanHistory={suratJalanHistory}
              vehicleItems={vehicleItems}
            />
          )}
        {/* --- RKBMD --- */}
        {activeTab === "rkbmd" && (
          <RkbmdView
            allowedBidangList={allowedBidangList}
            canEditRkbmd={canEditRkbmd}
            customAlert={customAlert}
            deleteRkbmd={deleteRkbmd}
            displayedRkbmdData={displayedRkbmdData}
            handleExportRkbmd={handleExportRkbmd}
            handleUploadRkbmdTemplate={handleUploadRkbmdTemplate}
            isDarkMode={isDarkMode}
            isReadOnly={isReadOnly}
            modals={modals}
            rkbmdArsip={rkbmdArsip}
            rkbmdFilters={rkbmdFilters}
            rkbmdSelectedJenis={rkbmdSelectedJenis}
            rkbmdSelectedYear={rkbmdSelectedYear}
            setModals={setModals}
            setPhotoModal={setPhotoModal}
            setRkbmdFilters={setRkbmdFilters}
            setRkbmdSelectedJenis={setRkbmdSelectedJenis}
            setRkbmdSelectedYear={setRkbmdSelectedYear}
            userRole={userRole}
          />
        )}
        {/* --- Inventaris Tanah --- */}
        {activeTab === "tanah" &&
          (userRole === "admin" ||
            (userRole === "admin_upt" && userUnit === UNIT_CODES.P2BTP)) && (
            <TanahView
              filteredTanahDataFinal={filteredTanahDataFinal}
              handleExportTanah={handleExportTanah}
              isDarkMode={isDarkMode}
              isReadOnly={isReadOnly}
              modals={modals}
              pbbSelectedYear={pbbSelectedYear}
              pbbYearsList={pbbYearsList}
              setEditTanahModal={setEditTanahModal}
              setMediaTanahModal={setMediaTanahModal}
              setModals={setModals}
              setPbbSelectedYear={setPbbSelectedYear}
              setPbbYearsList={setPbbYearsList}
              setPhotoModal={setPhotoModal}
              setStatus={setStatus}
              setTanahSearch={setTanahSearch}
              status={status}
              tanahSearch={tanahSearch}
              userRole={userRole}
            />
          )}
        {/* --- Riwayat Mutasi --- */}
        ...
        {activeTab === "monitoring" && (
          <MonitoringView
            activeMonitoringData={activeMonitoringData}
            data={data}
            handleExportRekapPengguna={handleExportRekapPengguna}
            isDarkMode={isDarkMode}
            isReadOnly={isReadOnly}
            monitoringSelectedYear={monitoringSelectedYear}
            priceData={priceData}
            setEditNilaiModal={setEditNilaiModal}
            setLinkAsetModal={setLinkAsetModal}
            setMonitoringSearch={setMonitoringSearch}
            setMonitoringSelectedYear={setMonitoringSelectedYear}
            setPhotoModal={setPhotoModal}
            templates={templates}
          />
        )}
        ...
        {activeTab === "penghapusan" &&
          (userRole === "admin" || userRole === "admin_upt") && (
            <PenghapusanView
              ...
              isDarkMode={isDarkMode}
              modals={modals}
              processPermanentDisposal={processPermanentDisposal}
              setModals={setModals}
              setMonitoringSearch={setMonitoringSearch}
            />
          )}
      ...
      {/* --- Global Modals --- */}
      {modals.bulkLabel && (
        <ModalBulkLabelExport
          onClose={() => setModals({ ...modals, bulkLabel: false })}
          monitoringData={activeMonitoringData}
          onExport={handleExportBulkLabel}
        />
      )}
      {modals.rkbmd && (
        <ModalRKBMD
          allowedBidangList={allowedBidangList}
          kodeBarangDataList={kodeBarangData}
          onClose={() => setModals({ ...modals, rkbmd: false })}
          onSave={handleAddRkbmd}
        />
      )}
      {modals.addItem && (
        <ModalAddAsset
          onClose={() => setModals({ ...modals, addItem: false })}
      ...
      {modals.addRoomItem && (
        <ModalAddRoomAsset
          onClose={() => setModals({ ...modals, addRoomItem: false })}
          onSave={handleAddRoomItem}
          priceData={filteredPriceData}
          ruanganData={ruanganData}
          pegawaiData={data}
          kodeBarangDataList={kodeBarangData}
      ...
      {modals.transferUnit && (
        <ModalTransferUnit
          item={transferUnitModal.item}
          sourceUnitView={activeUnitView}
          selectedUser={data.find((p) => p.nip === transferUnitModal.sourceRef)}
          selectedRoom={
            transferUnitModal.sourceType === "ruangan"
              ? transferUnitModal.sourceRef
              : null
          }
          onClose={() =>
            setTransferUnitModal({
          ...
          onSave={processTransferUnit}
          statusLoading={status.loading}
        />
      )}
      {modals.manualDisposal && (
        <ModalManualDisposal
          onClose={() => setModals({ ...modals, manualDisposal: false })}
          onSave={processManualDisposal}
          activeAssets={activeMonitoringData}
          statusLoading={status.loading}
        />
      )}
      {modals.manualLabel && (
        <ModalManualLabel
          onClose={() => setModals({ ...modals, manualLabel: false })}
          templates={templates}
          pegawaiData={data}
          monitoringData={monitoringDataFiltered}
        />
      )}
      {modals.feedback && (
        <ModalFeedback
          onClose={() => setModals({ ...modals, feedback: false })}
          onSave={handleSaveFeedback}
        />
      )}
      {linkAsetModal.show && (
        <ModalLinkAset
          onClose={() => setLinkAsetModal({ show: false, item: null })}
          onSave={processLinkAset}
          item={linkAsetModal.item}
          data={data}
          ruanganData={filteredRuanganData}
          statusLoading={status.loading}
        />
      )}
      {editNilaiModal.show && (
        <ModalEditNilaiBuku
          onClose={() => setEditNilaiModal({ show: false, item: null })}
          onSave={processEditNilai}
          item={editNilaiModal.item}
          statusLoading={status.loading}
          kodeBarangDataList={kodeBarangData}
        />
      )}
      {editPajakModal.show && (
        <ModalEditPajakOwner
          onClose={() => setEditPajakModal({ show: false, item: null })}
          onSave={processPajakOwner}
          item={editPajakModal.item}
          data={data}
          ruanganData={filteredRuanganData}
          statusLoading={status.loading}
        />
      ...
      {editTanahModal.show && (
        <ModalEditTanah
          show={editTanahModal.show}
          item={editTanahModal.item}
          onClose={() => setEditTanahModal({ show: false, item: null })}
          onSave={handleUpdateTanahInfo}
          statusLoading={status.loading}
        />
      )}
      ...
      {editRoom.show && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div
            className={`p-6 md:p-8 rounded-[2rem] w-full max-w-md shadow-2xl ${isDarkMode ? "bg-slate-800 text-white" : "bg-white"}`}
          >
            <h3 className="text-xl font-black mb-4">Edit Nama Ruangan</h3>
            <input
              type="text"
              className={`w-full p-4 border rounded-xl outline-none focus:border-emerald-500 font-bold mb-4 ${isDarkMode ? "bg-slate-900 border-slate-700 text-white" : "bg-slate-50"}`}
              value={editRoom.newName}
              onChange={(e) =>
                setEditRoom({ ...editRoom, newName: e.target.value })
              }
            />
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 py-3 font-bold rounded-xl ${isDarkMode ? "text-slate-400 hover:bg-slate-700" : "text-slate-500 hover:bg-slate-100"}`}
              >
                Batal
              </button>
              <button
                onClick={handleAddRoomName}
                disabled={
                  !editRoom.newName ||
                  editRoom.newName === editRoom.oldName ||
                  status.loading
                }
                className="flex-1 py-3 bg-emerald-600 text-white font-bold hover:bg-emerald-700 disabled:opacity-50 rounded-xl flex justify-center"
              >
                {status.loading ? (
      ...
      {modals.templateInfo && (
        <div className="fixed inset-0 z-[800] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in">
          <div
            className={`rounded-[2rem] w-full max-w-2xl overflow-hidden shadow-2xl p-6 md:p-8 max-h-[90vh] overflow-y-auto ${isDarkMode ? "bg-slate-800 text-white" : "bg-white"}`}
          >
            ...
            >
              Anda dapat membuat file <code>.html</code> atau <code>.txt</code>{" "}
              lalu mengunggahnya. Sistem akan mengganti kata kunci (placeholder)
              berikut secara otomatis saat cetak:
            </p>
            <div className="space-y-4 text-xs">
              <div
                className={`p-4 rounded-xl border ${isDarkMode ? "bg-slate-900 border-slate-700" : "bg-slate-50"}`}
              >
                <h4 className="font-bold text-sm text-indigo-500 mb-2">
                  Cetak SPPBI
                </h4>
                ...
                <p className="mt-2 text-slate-500">
                  Gunakan {"{{TABLE_ROWS}}"} di dalam tag &lt;tbody&gt; untuk
                  menyisipkan baris aset.
                </p>
              </div>
              <div
                className={`p-4 rounded-xl border ${isDarkMode ? "bg-slate-900 border-slate-700" : "bg-slate-50"}`}
              >
                ...
                <p className="font-mono text-slate-400">
                  {"{{ROOM_NAME}}"}, {"{{KASUBAG_NAMA}}"}, {"{{KASUBAG_NIP}}"},{" "}
                  {"{{PENGURUS_NAMA}}"}, {"{{PENGURUS_NIP}}"}, {"{{TANGGAL}}"},{" "}
                  {"{{TABLE_ROWS}}"}
                </p>
              </div>
              <div
                className={`p-4 rounded-xl border ${isDarkMode ? "bg-slate-900 border-slate-700" : "bg-slate-50"}`}
              >
                <h4 className="font-bold text-sm text-amber-500 mb-2">
                  Cetak Label Aset
                </h4>
                <p className="font-mono text-slate-400">
                  {"{{NAMA_BARANG}}"}, {"{{MERK}}"}, {"{{NO_KARTU}}"},{" "}
                  {"{{TAHUN}}"}, {"{{SISTEM_ID}}"}, {"{{LOKASI}}"},{" "}
                  {"{{NAMA_DINAS}}"}, {"{{NAMA_SKPD}}"}
                </p>
                <p className="mt-2 text-slate-500">
                  NAMA_DINAS akan selalu menjadi "Dinas Perkebunan Prov. Jatim",
                  sedangkan NAMA_SKPD akan ditarik dari data SKPD/Bidang pemilik
                  barang.
                </p>
              </div>
            </div>
            <button
              onClick={() => setModals({ ...modals, templateInfo: false })}
              className={`mt-8 w-full py-3 font-bold rounded-xl ${isDarkMode ? "bg-slate-700 hover:bg-slate-600 text-white" : "bg-slate-100 hover:bg-slate-200 text-slate-600"}`}
            >
              Tutup Panduan
            </button>
          </div>
      ...
      {photoModal.show && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <div
            className={`p-6 md:p-8 rounded-3xl w-full max-w-lg text-center max-h-[90vh] overflow-hidden flex flex-col shadow-2xl ${isDarkMode ? "bg-slate-800" : "bg-white"}`}
          >
            <div className="flex justify-between items-center mb-6">
              <h3
                className={`text-xl font-black ${isDarkMode ? "text-white" : ""}`}
              >
                {photoModal.isSPPBI
                  ? "Dokumen SPPBI"
                  : photoModal.isPajak
                    ? `Bukti Pajak ${photoModal.pajakYear}`
                    : photoModal.isPBB
                      ? `Bukti PBB ${photoModal.pbbYear}`
                      : photoModal.isRkbmdArchive
                        ? `Arsip ${photoModal.rkbmdMeta.jenis} (${photoModal.rkbmdMeta.tahun})`
                        : "Foto Aset"}
              </h3>
              <button
                onClick={() =>
                ...
                className={`p-2 rounded-xl ${isDarkMode ? "bg-slate-700 text-slate-300 hover:bg-slate-600" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}
              >
                <X size={20} />
              </button>
            </div>
            {photoModal.preview && !photoModal.file ? (
              <div
                className={`w-full rounded-2xl mb-6 overflow-hidden relative flex flex-col items-center justify-center min-h-[300px] border p-4 ${isDarkMode ? "bg-slate-900 border-slate-700" : "bg-slate-100 border-slate-200"}`}
              >
                {photoModal.isPdf ? (
                  <iframe
              ...
              <div className="mb-6">
                <label
                  className={`w-full aspect-video rounded-2xl overflow-hidden relative group cursor-pointer border-2 border-dashed flex items-center justify-center transition-colors hover:border-emerald-500 ${isDarkMode ? "bg-slate-900 border-slate-600" : "bg-slate-50 border-slate-300"}`}
                >
                  <input
                    type="file"
                  ...
                  {photoModal.preview ? (
                    photoModal.isPdf ? (
                      <div className="flex flex-col items-center">
                        ...
                        <span className="font-bold">{photoModal.fileName}</span>
                      </div>
                    ) : photoModal.isExcel ? (
                      <div className="flex flex-col items-center">
                        <FileType size={48} className="text-emerald-500 mb-2" />
                        <span className="font-bold">{photoModal.fileName}</span>
                      </div>
                    ) : (
                      <img
                    ...
                    <div className="flex flex-col items-center gap-3 text-slate-400 font-bold group-hover:text-emerald-500 p-6">
                      ...
                      <span className="text-sm">
                        ...
                        <span className="text-xs font-normal mt-1 block">
                          Mendukung Foto (JPG/PNG), PDF, Excel
                        </span>
                      </span>
                    </div>
                  )}
            ...
            {(photoModal.file || (!photoModal.preview && !isReadOnly)) && (
              <button
                onClick={savePhotoToCloud}
                disabled={!photoModal.file || status.loading}
                className="w-full py-4 bg-emerald-600 text-white rounded-xl font-black hover:bg-emerald-700 disabled:bg-slate-300 transition-all text-sm md:text-base flex justify-center items-center gap-2 shadow-lg"
              >
                {status.loading ? (
                  <>
      ...
      {modals.upload && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 md:p-8 bg-slate-900/90 backdrop-blur-3xl">
          <div
            className={`rounded-[2rem] md:rounded-[5rem] shadow-2xl w-full max-w-4xl overflow-hidden border flex flex-col max-h-[90vh] ${isDarkMode ? "bg-slate-800 border-slate-700" : "bg-white"}`}
          >
            <div
              className={`p-6 md:p-10 border-b flex justify-between items-center shrink-0 ${isDarkMode ? "bg-slate-800/50 border-slate-700" : "bg-slate-50/50"}`}
            >
              <h3
                className={`text-xl md:text-3xl font-black uppercase tracking-tighter flex items-center gap-2 md:gap-4 ${isDarkMode ? "text-white" : ""}`}
              >
                <Database className="w-6 h-6 md:w-8 md:h-8" /> Sync DB Master
              </h3>
              <button
                onClick={() => setModals({ ...modals, upload: false })}
                className="p-2 md:p-4 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-xl transition-all"
              >
                <X size={32} />
              </button>
            </div>
            <div className="p-6 md:p-12 text-center overflow-y-auto">
              <div className="grid grid-cols-2 md:grid-cols-5 gap-2 md:gap-4 mb-8">
                {userRole === "admin" && (
                  <button
                    onClick={() =>
                      setStatus({ ...status, uploadType: "pegawai" })
                    }
                    className={`p-3 md:p-4 rounded-xl md:rounded-2xl font-black text-[10px] md:text-sm uppercase tracking-widest transition-all ${status.uploadType === "pegawai" ? "bg-slate-900 dark:bg-slate-600 text-white shadow-xl scale-105" : "bg-slate-100 dark:bg-slate-900 text-slate-400 hover:bg-slate-200"}`}
                  >
                    Data Pegawai
                  </button>
                )}
                <button
                  onClick={() => setStatus({ ...status, uploadType: "prices" })}
                  className={`p-3 md:p-4 rounded-xl md:rounded-2xl font-black text-[10px] md:text-sm uppercase tracking-widest transition-all ${status.uploadType === "prices" ? "bg-emerald-600 text-white shadow-xl scale-105" : "bg-slate-100 dark:bg-slate-900 text-slate-400 hover:bg-slate-200"}`}
                >
                  Nilai Buku
                </button>
                {userRole === "admin" && (
                  <button
                    onClick={() =>
                      setStatus({ ...status, uploadType: "kode_barang" })
                    }
                    className={`p-3 md:p-4 rounded-xl md:rounded-2xl font-black text-[10px] md:text-sm uppercase tracking-widest transition-all ${status.uploadType === "kode_barang" ? "bg-indigo-600 text-white shadow-xl scale-105" : "bg-slate-100 dark:bg-slate-900 text-slate-400 hover:bg-slate-200"}`}
                  >
                    Master Kode
                  </button>
                )}
                {userRole === "admin" && (
                  <button
                    onClick={() =>
                      setStatus({ ...status, uploadType: "pajak" })
                    }
                    className={`p-3 md:p-4 rounded-xl md:rounded-2xl font-black text-[10px] md:text-sm uppercase tracking-widest transition-all ${status.uploadType === "pajak" ? "bg-purple-600 text-white shadow-xl scale-105" : "bg-slate-100 dark:bg-slate-900 text-slate-400 hover:bg-slate-200"}`}
                  >
                    Pajak Motor
                  </button>
                )}
                {userRole === "admin" && (
                  <button
                    onClick={() =>
                      setStatus({ ...status, uploadType: "tanah" })
                    }
                    className={`p-3 md:p-4 rounded-xl md:rounded-2xl font-black text-[10px] md:text-sm uppercase tracking-widest transition-all ${status.uploadType === "tanah" ? "bg-amber-600 text-white shadow-xl scale-105" : "bg-slate-100 dark:bg-slate-900 text-slate-400 hover:bg-slate-200"}`}
                  >
                    Tanah (KIB A)
                  </button>
                )}
              ...
              <div className="flex flex-col items-center mb-6">
                <label className="flex items-center gap-3 p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-2xl cursor-pointer hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors text-left max-w-2xl">
                  <input
                    type="checkbox"
                    checked={status.uploadAppend}
                    onChange={(e) =>
                      setStatus({ ...status, uploadAppend: e.target.checked })
                    }
                    className="w-5 h-5 accent-emerald-600 mt-1"
                  />
                  <span className="text-sm font-bold text-emerald-800 dark:text-emerald-200">
                    Mode Tambah Data (Append) - Sangat Aman
                    <br />
                    <span className="text-xs font-normal opacity-90 mt-1 block">
                      Aset atau data baru yang Anda upload melalui Excel{" "}
                      <b>TIDAK</b> akan menghapus data yang sudah ada dari
                      tahun-sebelumnya. Sistem akan otomatis menyisipkan data
                      baru tanpa menimpa arsip yang lama.
                    </span>
                  </span>
                </label>
              </div>
              <label
                htmlFor="portalSync"
                className={`block border-[4px] md:border-[12px] border-dashed rounded-[2rem] md:rounded-[4rem] p-8 md:p-16 transition-all duration-700 cursor-pointer group ${status.loading ? "opacity-20 animate-pulse bg-emerald-50" : "border-emerald-100 dark:border-emerald-900/30 hover:border-emerald-500"}`}
              >
                <input
                  type="file"
                  accept=".xlsx, .xls, .csv"
                ...
                <div className="flex flex-col items-center">
                  ...
                  >
                    {status.uploadType === "pegawai" ? (
                      <Users size={48} />
                    ) : status.uploadType === "kode_barang" ? (
                      <ListTree size={48} />
                    ) : status.uploadType === "pajak" ? (
                      <Car size={48} />
                    ) : status.uploadType === "tanah" ? (
                      <MapPin size={48} />
                    ) : (
                      <DollarSign size={48} />
                    )}
                  ...
                  <p
                    className={`text-lg md:text-2xl font-black uppercase tracking-tighter leading-none ${isDarkMode ? "text-white" : "text-slate-800"}`}
                  >
                    {status.loading
                      ? `UPDATING ${status.progress}%`
                      : `UPLOAD FILE ${status.uploadType.toUpperCase()}`}
                  </p>
                  <p className="text-slate-400 mt-2 text-sm font-bold">
                    Klik area ini untuk memilih dokumen Excel
                  </p>
                </div>
              </label>
            </div>
      ...
      {confirmModal.show && (
        <div className="fixed inset-0 z-[900] flex items-center justify-center p-4 md:p-8 bg-slate-900/90 backdrop-blur-sm animate-in fade-in">
          <div
            className={`rounded-[2rem] md:rounded-[3rem] shadow-2xl w-full max-w-md overflow-hidden text-center p-8 md:p-12 border ${isDarkMode ? "bg-slate-800 border-slate-700" : "bg-white"}`}
          >
            <div
              className={`w-20 h-20 md:w-24 md:h-24 rounded-full flex items-center justify-center mx-auto mb-6 ${confirmModal.type === "danger" ? "bg-red-50 text-red-500 dark:bg-red-900/30" : "bg-emerald-50 text-emerald-500 dark:bg-emerald-900/30"}`}
            >
            ...
            <h3
              className={`text-2xl md:text-3xl font-black uppercase tracking-tighter mb-4 ${isDarkMode ? "text-white" : "text-slate-800"}`}
            >
              {safeString(confirmModal.title)}
            </h3>
            <p className="text-slate-500 dark:text-slate-300 font-bold mb-8 text-sm md:text-base">
              {safeString(confirmModal.message)}
            </p>
            ...
            {confirmModal.isAlert ? (
              <button
                ...
                className="w-full py-3 md:py-4 bg-emerald-600 text-white rounded-xl md:rounded-2xl font-black uppercase hover:bg-emerald-700 transition-all text-xs md:text-sm"
              >
                Oke Mengerti
              </button>
            ) : (
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 py-3 md:py-4 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl md:rounded-2xl font-black uppercase hover:bg-slate-200 transition-all text-xs md:text-sm"
                >
                  Batal
                </button>
                <button
                  ...
                  className={`flex-1 py-3 md:py-4 text-white rounded-xl md:rounded-2xl font-black uppercase transition-all flex justify-center items-center gap-2 text-xs md:text-sm ${confirmModal.type === "danger" ? "bg-red-600 hover:bg-red-700" : "bg-emerald-600 hover:bg-emerald-700"}`}
                >
                  Lanjutkan
                </button>
              </div>
